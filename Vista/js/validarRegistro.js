...
function validarFormulario() {
  ...
  if (!validaciones.nombre.test(apellido)) {
  ...
  var ci = $("#ci").val();
  if (ci.length == 0) {
    alert("Escriba una C.I. o RUC valido");
    return false;
  }
  if (!validarCedula(ci)) {
    alert("Cedula de Identidad o RUC incorrecto");
    return false;
  }
  ...
  if (!validaciones.direccion.test(direccion)) {
  ...
  if (!validaciones.telefono.test(telefono)) {
  ...
  let pattern2=new RegExp("(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}");
  var contrasenia = $("#contrasenia").val();
  if (contrasenia.length == 0) {
    alert("Escriba una contraseña valida");
    return false;
  }
  if (!contrasenia.match(/[a-z]/g) || !contrasenia.match(/[A-Z]/g) || !contrasenia.match(/[0-9]/g) || !contrasenia.length >=8) {
